...
print("="*70)
print("RANDOM FOREST - PREDICTION SCRIPT")
print("="*70)
...
# Set to None to predict on all data
...
try:
    ...
    print("   Please train the model first using train_random_forest_standalone.py")
    exit(1)
...
if len(feature_columns) != rf_model.n_features_in_:
    print(f"❌ Error: Feature mismatch!")
    print(f"   Model expects {rf_model.n_features_in_} features")
    print(f"   Data has {len(feature_columns)} features")
    exit(1)
...
probabilities = rf_model.predict_proba(X)[:, 1]  # Probability of Class 1
...
if has_labels:
    ...
    if has_labels:
        ...
        if sum(correct_mask) > 0:
            max_conf_correct = probabilities[correct_mask].max() if sum((predictions == 1) & correct_mask) > 0 else (1 - probabilities[correct_mask].min())
            print(f"  • Most confident correct prediction: {max_conf_correct:.4f}")
        
        if sum(incorrect_mask) > 0:
            # For incorrect, high probability when wrong is bad
            max_conf_incorrect = probabilities[incorrect_mask & (predictions == 1)].max() if sum(incorrect_mask & (predictions == 1)) > 0 else (1 - probabilities[incorrect_mask & (predictions == 0)].min())
            print(f"  • Most confident incorrect prediction: {max_conf_incorrect:.4f}")
